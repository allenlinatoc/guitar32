            ...
            if (this.realtimeValidation) {
                ...
                if (this.toolTip != null && (this.requiredField ? ctrl.Text.Trim().Length > 0 : true)) {
                    if (!Regex.IsMatch(this.control.Text, this.expression)) {
                        ...
                        // Fix for `System.ObjectDisposedException`
                        try
                        {
                            this.toolTip.Hide(ctrl);
                        }
                        ...
                        {
                            // Log to event file
                            Utilities.Diagnostics diagnose = new Utilities.Diagnostics();
                            diagnose.LogEntry(ex.Message, System.Diagnostics.EventLogEntryType.Warning);
                        }
                    }
                }
            }
